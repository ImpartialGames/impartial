...
serve(async (req) => {
  ...
  try {
    const { email, prenom, factureRef, montant } = await req.json();
    ...
    await resend.emails.send({
      ...
      to: [email],
      subject: `Paiement confirmé — ${factureRef}`,
      html: `
<!DOCTYPE html>
<html lang="fr">
...
<body style="margin:0;padding:0;background:#09090b;font-family:'Segoe UI',Roboto,Arial,sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#09090b;">
      ...
      <table role="presentation" width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;">
        ...
        <tr><td style="background:#111116;border:1px solid #27272a;border-radius:16px;padding:36px 32px;">
          ...
          <div style="text-align:center;margin-bottom:24px;">
            <div style="display:inline-block;background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.25);border-radius:50%;width:56px;height:56px;line-height:56px;font-size:28px;">✓</div>
          </div>

          <p style="margin:0 0 4px;font-size:13px;color:#22c55e;font-weight:600;text-transform:uppercase;letter-spacing:1.5px;text-align:center;">Paiement reçu</p>
          <h2 style="margin:0 0 20px;font-size:20px;color:#fafafa;font-weight:600;text-align:center;">Merci ${clientPrenom} !</h2>
          <p style="margin:0 0 24px;font-size:14px;line-height:1.7;color:#a1a1aa;text-align:center;">
            Nous confirmons la bonne réception de votre paiement. Voici le récapitulatif :
          </p>
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="padding-bottom:14px;">
                    <p style="margin:0 0 4px;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#71717a;">Facture</p>
                ...
                <tr>
                  <td style="border-top:1px solid #1e1e2a;padding:14px 0;">
                    <p style="margin:0 0 4px;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#71717a;">Montant réglé</p>
                    <p style="margin:0;font-size:22px;color:#22c55e;font-weight:700;">${Number(montant).toLocaleString("fr-FR")} €</p>
                  </td>
                </tr>
                <tr>
                  <td style="border-top:1px solid #1e1e2a;padding-top:14px;">
                    <p style="margin:0 0 4px;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#71717a;">Date</p>
                    <p style="margin:0;font-size:15px;color:#fafafa;font-weight:500;">${new Date().toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</p>
                  </td>
                </tr>
              </table>
            </td></tr>
          ...
          <p style="margin:24px 0 0;font-size:13px;color:#71717a;line-height:1.6;text-align:center;">
            Merci pour votre confiance. Nous restons à votre disposition pour tout besoin.
          </p>
        </td></tr>
    ...
    console.error("Error in send-paiement:", error);
