...
serve(async (req) => {
  ...
  try {
    const { email, prenom, demandeRef, titre } = await req.json();

    if (!email || !demandeRef || !titre) {
      throw new Error("Email, référence et titre sont obligatoires");
    }
    ...
    await resend.emails.send({
      ...
      to: [email],
      subject: `Demande reçue — ${demandeRef}`,
      html: `
<!DOCTYPE html>
<html lang="fr">
...
<body style="margin:0;padding:0;background:#09090b;font-family:'Segoe UI',Roboto,Arial,sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#09090b;">
    <tr><td align="center" style="padding:40px 16px;">
      <table role="presentation" width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;">
        ...
        <tr><td style="background:#111116;border:1px solid #27272a;border-radius:16px;padding:36px 32px;">
          <p style="margin:0 0 4px;font-size:13px;color:#a78bfa;font-weight:600;text-transform:uppercase;letter-spacing:1.5px;">Demande reçue</p>
          ...
          <p style="margin:0 0 24px;font-size:14px;line-height:1.7;color:#a1a1aa;">
            Nous avons bien reçu votre demande et notre équipe l'examine actuellement. Nous reviendrons vers vous dans les meilleurs délais.
          </p>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:rgba(167,139,250,0.06);border:1px solid rgba(167,139,250,0.15);border-radius:10px;margin-bottom:28px;">
            <tr><td style="padding:14px 18px;">
              <p style="margin:0;font-size:13px;color:#a1a1aa;line-height:1.5;">
                📋 Vous pouvez suivre l'avancement de votre demande à tout moment depuis votre espace client.
              </p>
            </td></tr>
          </table>
    ...
    console.error("Error in send-demande-reception:", error);
